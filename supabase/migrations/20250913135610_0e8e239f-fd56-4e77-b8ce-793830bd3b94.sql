-- Fix database schema inconsistencies and add missing tables
...
-- Standardize on 'inventory' table (dropping room_inventory if it conflicts)
-- Update inventory table to ensure it has all necessary columns
...
-- Add RLS policies for agencies
ALTER TABLE public.agencies ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "agencies_org_access" ON public.agencies
FOR ALL USING (org_id IN (SELECT org_id FROM users WHERE auth_user_id = auth.uid()));

-- Add RLS policies for guests
ALTER TABLE public.guests ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "guests_rw" ON public.guests
...
-- Add RLS policies for rate_plans
